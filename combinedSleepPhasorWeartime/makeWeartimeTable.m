function [wearTime,unWornTime,percentWorn,percentUnworn,oneLine,nonCompliantOneLine] = makeWeartimeTable(absTime,relTime,epoch,light,activity,masks,subjectID,cdfData)
%generate data for a subject and then store it in the preallocated cells
    ...
    wearTime = useTime;
    unWornTime = nonuseTime; 
    percentWorn = usePrcnt; 
    percentUnworn = nonusePrcnt;
...
function [useTime,nonuseTime,usePrcnt,nonusePrcnt] = deviceUseMinutes(masks,epoch)
...
useTime = nUse*epoch.minutes;
nonuseTime = nNonuse*epoch.minutes;
